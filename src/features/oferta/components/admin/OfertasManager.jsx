...
const OfertasList = () => {
  ...
  // Consumimos nuestro hook estandarizado
  const { data: programas = [], isLoading, error, refetch } = useAdminOfertas();

  // Filtrado local sobre los datos en caché
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {error && (
        <div className="mb-8 p-6 bg-red-50 border border-red-100 rounded-2xl flex flex-col items-center justify-center text-center animate-in zoom-in-95">
          ...
          <p className="text-xs text-red-600 mb-4">{error.message || "No pudimos cargar el catálogo de programas. Verifica tu conexión."}</p>
          <button 
            onClick={() => refetch()}
              ...
              {!isLoading && !error && filteredProgramas.length === 0 ? (
                ...
                filteredProgramas.map((programa) => (
                  <tr key={programa.id} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    {/* Columna: Información del Programa */}
                    ...
                    <td className="px-6 py-5 text-center">
                      {programa.isTop ? (
                        ...
                        <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-slate-50 text-slate-300">
                          <span className="material-symbols-rounded text-sm">star_border</span>
                        </span>
                      )}
                    </td>
