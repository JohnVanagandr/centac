...
const SolicitudesTable = ({ data }) => {
  // Renderizado seguro en caso de que data no sea un arreglo válido
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {data.map((sol) => {
              const folio = sol.id;
              const aprendiz = sol.full_name || sol.aprendiz || "Sin Nombre";
              const programa = sol.program?.title || sol.programa || "No especificado";
              const estado = sol.status || sol.estado || "Pendiente";
      ...
      ${estado === "Pendiente" ? "bg-amber-50 text-amber-600 border-amber-100" : ""}
      ${estado === "Contactado" ? "bg-blue-50 text-blue-600 border-blue-100" : ""}
      ${estado === "Matriculado" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : ""}
      ${estado === "No Interesado" ? "bg-slate-50 text-slate-400 border-slate-200" : ""}
    `}
                    >
                      {estado}
                    </span>
